#!/bin/bash
# Baseline matching chengnuojin's DeepSeek-V3 8x8x8 config (FSDP128 x EP8, ep-as-dp,
# ring+ragged+chunk2+rbf2, autodiff bwd, splash-1024 blocks, THEIR XLA flags).
# On bodaborg-super-tpu7x-y6k at 8x8x8 (512 chips) via xpk super-slicing.
set -e
set -o pipefail
...
attention=flash max_target_length=4096 \
use_random_routing=true use_ring_of_experts=true use_ragged_sort=true \
tokenizer_path=assets/tokenizer.mistral-v3 merge_gating_gmm=false \
enable_tpu_profiling_options=true \
base_output_directory=gs://sivaibhav-exp/1410-a2a run_name=${NAME} \
...
echo "=== launching $NAME : 8x8x8 (512 chips) super-slicing, priority very-high ==="
SUB_SLICING_ENABLED=true xpk workload create \
  --cluster=$CLUSTER --project=$PROJECT --zone=$ZONE \
  --device-type=tpu7x-8x8x8 --num-slices=1 \
  --reservation=$RES \
