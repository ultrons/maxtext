...
ragged_buffer_factor=-1 num_moe_token_chunks=2 moe_fast_group_topk=false"
...
IMGTAG=gtopk ENVX="MOE_UNSORT_BWD_MASK=0" bash xpk_variant_poison.sh "$SUF" \
  "$SHIP $DATA $CKPT $MODEL $LR $EVAL $SAFE steps=10"
