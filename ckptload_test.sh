#!/bin/bash
# Restore probe: can ckpt0424-fsdp (written fsdp=16/ep=1) load into our fsdp=128/ep=8 mesh,
# and how long does it take? MTP on (checkpoint carries mtp_block). Grouped routing OFF
# (no param-shape effect; isolate the restore). chunks=1 per house rule.
set -e
SUF="${1:-ckptload1}"
...
CKPT="enable_checkpointing=True async_checkpointing=false checkpoint_period=100000 \
...
bash xpk_variant.sh "$SUF" "$SHIP $DATA $CKPT $MODEL $LR steps=12"
