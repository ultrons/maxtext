...
class SplashConfig:
  ...
  # REDUCE WASTED DIAGONAL MATMUL in the fused backward: on a partial-mask
  # (diagonal) causal block, split the QK matmul into a qk_diag_grid x qk_diag_grid
  # sub-grid over (kv rows, q cols). Every sub-tile with row-band ki > col-band qj
  # is entirely above the causal boundary (kv > q) on an aligned SQUARE diagonal
  # block, so its scores are overwritten to mask_value by _apply_mask_and_soft_cap
  # regardless of value -> skip its matmul, fill mask_value, assemble the full score
  # tile, then the SAME single exp/ds over the full tile (elementwise/downstream
  # unchanged). PRECONDITION: pure causal mask + aligned square blocks
  # (bq == bkv_compute). Verify bit-exact vs qk_diag_skip=False before trusting.
  qk_diag_skip: bool = False
  # Granularity of the diagonal skip: grid=2 -> skip 1/4, grid=4 -> skip 6/16.
  # Bigger grid skips more of the triangle but uses smaller matmuls -> measure net.
  qk_diag_grid: int = 2
...
def flash_attention_kernel(
  ...
  def body(kv_compute_index, _, has_partial_mask=False):
    ...
    exp = jnp.exp2 if config.use_base2_exp else jnp.exp
    if max_logit_estimate is None:
      s_curr = exp(qk - jnp.tile(m_next, (1, 1, bkv_repeats)))
    else:
    ...
    if max_logit_estimate is None:
      alpha_o = jnp.tile(alpha, (1, 1, head_dim_v_repeats))
...
def _flash_attention_dkv_kernel(
  ...
  def body(i, _, has_partial_mask=False):
    ...
    if (
      ...
      # Diagonal causal block: g x g sub-grid over (kv rows, q cols). Every sub-tile
      # with row-band ki > col-band qj is fully above the causal boundary (kv > q) on
      # an aligned square diagonal block -> its scores are overwritten to mask_value
      # by _apply_mask_and_soft_cap anyway, so SKIP its matmul and fill mask_value.
      # Compute only ki <= qj sub-tiles; assemble the full tile for the single exp/ds.
      sk = bkv_compute // _g
      sq = bq // _g
      k_parts = [k[bi * sk:(bi + 1) * sk, :] for bi in range(_g)]
      q_parts = [scaled_q[bj * sq:(bj + 1) * sq, :] for bj in range(_g)]
