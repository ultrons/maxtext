...
class MoEGeneral(BaseModel):
  ...
  decouple_combine_rs_chunks: int = Field(
      ...
      description=(
          ...
          "num_moe_token_chunks, which chunks the whole body INCL the GMM (caps ~2 on GMM efficiency); here the GMM is "
  ...
  moe_shared_after_combine: bool = Field(
      ...
      description=(
          ...
          "decouple_combine_rs_chunks>1 + use_ring_of_experts, single-axis expert, num_moe_token_chunks<=1): fence the "
...
class MaxTextConfig(
  ...
  def set_derived_and_validate_values(self) -> "MaxTextConfig":
    ...
    if self.num_experts > 1:
      ...
      if getattr(self, "record_expert_histogram", False) and self.routed_bias and self.routed_bias_update_rate > 0.0:
        raise ValueError(
            "record_expert_histogram reuses the bias_updates channel and conflicts with ACTIVE "
            "loss-free balancing; set routed_bias_update_rate=0 while recording."
        )
