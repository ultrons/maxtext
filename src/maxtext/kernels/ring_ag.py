...
Vendored from perf-drills/gather/ring_ag/ring_ag.py (v7x-validated: tiled semantics bit-exact vs
``jax.lax.all_gather(..., tiled=True)``; single-axis bidi 159-179 GB/s; RS-twin validated rel=0),
adapted for IN-shard_map use: `ring_all_gather` runs the pallas_call directly on the local shard
inside an enclosing shard_map (the perf-drills `build_ring_ag` wraps its own jit+shard_map, which
cannot nest inside the MoE shard_map), with a caller-supplied `collective_id` (the perf-drills
kernel hardcodes 0, which would collide with other in-flight Pallas collectives' barrier
semaphores) and an explicit `CostEstimate` so the latency-hiding scheduler can see the ~2x-bytes
