...
# ---------------------------------------------------------------------------
# TensorCore post-pass: cast only the rows that own a valid source row.
# ---------------------------------------------------------------------------

# Output rows per grid step of the masked cast; 128 rows x 7168 f32 is 3.7 MB
# per buffer, double-buffered, well inside the 64 MiB TensorCore VMEM.
_CAST_TILE_ROWS = 128
_CAST_ROW_UNROLL = 8
...
def _masked_rows_to_bf16_kernel(
  ...
  """Grid step ``i``: rows ``[i * tile_rows, (i + 1) * tile_rows)`` of the output.
  ...
  def fetch(tile, s):
    ...
    def issue(k, carry):
      for u in range(row_unroll):
        row = valid_rows_ref[base + k * row_unroll + u]
        pltpu.make_async_copy(
            x_hbm_ref.at[pl.ds(row, 1), :],
            buf_ref.at[s, pl.ds(row - base, 1), :],
            sem_ref.at[s],
        ).start()
      return carry

    jax.lax.fori_loop(0, counts_ref[tile], issue, 0)

  def wait(tile, s):
    def one(k, carry):
      for _ in range(row_unroll):
        pltpu.make_async_copy(
            x_hbm_ref.at[pl.ds(0, 1), :],
            buf_ref.at[s, pl.ds(0, 1), :],
            sem_ref.at[s],
        ).wait()
      return carry

    jax.lax.fori_loop(0, counts_ref[tile], one, 0)
...
def _masked_rows_to_bf16(
    ...
    out_dtype: Any,
    *,
    tile_rows: int = _CAST_TILE_ROWS,
    row_unroll: int = _CAST_ROW_UNROLL,
    interpret: Any = False,
) -> jax.Array:
  """``where(mask[:, None], x[:num_rows, :hidden_size], 0).astype(out_dtype)`` reading only the masked-in rows.

  ``x`` is the SparseCore kernel's float32 output (padded rows, plus the
  garbage row); only rows with ``mask`` set own a valid source row, and in the
  DeepSeek-V3 recipe that is about 40% of them, so this pass reads 40% of the
  bytes the fused slice+select+convert did. The cast is the same TensorCore
  convert, on the same float32 sums.
  """
  ...
  # multiple of ``row_unroll`` with repeats of the tile's last valid row (a
  # repeated DMA of the same row to the same place is harmless); tiles with no
  # valid row get a count of zero and never issue a DMA.
  tile_mask = mask.reshape(num_tiles, tile_rows)
  order = jnp.argsort(~tile_mask, axis=-1, stable=True)
  counts = jnp.sum(tile_mask, axis=-1).astype(jnp.int32)
  padded_counts = ((counts + row_unroll - 1) // row_unroll).astype(jnp.int32)
  slot = jnp.arange(tile_rows, dtype=jnp.int32)[None, :]
  ...
  return pl.pallas_call(
      functools.partial(_masked_rows_to_bf16_kernel, tile_rows=tile_rows, row_unroll=row_unroll),
      grid_spec=grid_spec,
      out_shape=jax.ShapeDtypeStruct((num_rows, hidden_size), out_dtype),
      ...
      cost_estimate=pl.CostEstimate(
          ...
          bytes_accessed=num_rows * hidden_size * (4 + jax.dtypes.itemsize_bits(out_dtype) // 8),
          transcendentals=0,
      ),
      interpret=interpret,
...
def ragged_gather_reduce(
  ...
  # The SparseCore kernel writes float32; casting on the SparseCore costs more
  # vector work than the TensorCore pass it removes (v7x: +82% kernel time),
  # so the cast stays on the TensorCore, but reads only the rows of non-empty
  # groups instead of streaming the whole float32 buffer.
  num_output_rows = input_size // reduce_group_size
  if num_output_rows % _CAST_TILE_ROWS == 0 and aligned_hidden_size == hidden_size:
    return _masked_rows_to_bf16(out, mask, num_output_rows, hidden_size, x.dtype)
  out = out[:num_output_rows, :hidden_size]
  out = jnp.where(mask[:num_output_rows, None], out, jnp.zeros_like(out))
