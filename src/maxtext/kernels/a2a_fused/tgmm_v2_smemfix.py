...
import os, sys  # SMEM-FIX: import the forked gmm kernel (O(num_groups) metadata)
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from . import gmm_v2_smemfix as gmm_v2
...
def get_cost_estimate(cfgs: gmm_v2.GmmConfigs) -> pl.CostEstimate:
  ...
  return pl.CostEstimate(
      flops=flops // 1000,
      bytes_accessed=(lhs_bytes + rhs_bytes + out_bytes) // 1000,
      transcendentals=0,
  )
...
def tgmm_inner_kernel(
  ...
  def _matmul(is_new_group: bool, is_group_changing: bool):
    ...
    _, m_start, m_end = gmm_v2.derive_tile(metadata_ref, gm_id, cfgs)
  ...
  is_first_gm = gm_id == 0
  cur_group_id, _, _ = gmm_v2.derive_tile(metadata_ref, gm_id, cfgs)
  prev_group_id, _, _ = gmm_v2.derive_tile(metadata_ref, prev_gm_id, cfgs)
  group_id_changed = cur_group_id != prev_group_id
  ...
  next_group_id, _, _ = gmm_v2.derive_tile(metadata_ref, next_gm_id, cfgs)
...
class TgmmIndexMaps:
  ...
  def lhs_index_map(self, n_id: jax.Array, k_id: jax.Array, gm_id: jax.Array):
    _, m_start, m_end = gmm_v2.derive_tile(self.metadata_ref, gm_id, self.cfgs)
  ...
  def out_index_map(self, n_id: jax.Array, k_id: jax.Array, gm_id: jax.Array):
    group_id, _, _ = gmm_v2.derive_tile(self.metadata_ref, gm_id, self.cfgs)
...
def zero_out_start(
  ...
  COMPILE-FIX (2026-07-13): the group loop is a RUNTIME ``lax.fori_loop`` instead of
  a Python ``range`` unroll. Previously this emitted one ``make_async_copy`` per
  (group, k-chunk, n-lane) STATICALLY -> ~num_actual_groups * cdiv(k,tile_zero_k) *
  (n/num_lanes) ops (~115K at g=64,D=7168,F=2048), and Mosaic lowering is super-
  linear in static op count, so tgmm compile was ~O(g^1.7) (264s at g=64, freezing
  >40min at the fused prod shape). Emitting the copy body once inside a runtime loop
  makes the op count independent of num_actual_groups. The inner k/n loops stay
  STATIC (they need static copy sizes; only the leading 0/1 row count and the group
  index are dynamic -- both already dynamic-slice-legal for make_async_copy).
  Behaviour is byte-for-byte identical: only empty groups (group_sizes[i]==0) are
  zeroed, and the byte-count drain in zero_out_end is unchanged.
  """
  ...
  def fill_zero(local_group_id, should_copy_int):
    # STATIC k/n chunk loops -> copy sizes are Python ints (dynamic-size DMA in a
    # non-leading dim is unsupported). Body is emitted ONCE (inside the runtime
    # group loop), so total static ops = cdiv(k,tile_zero_k) * (n/num_lanes),
    # independent of num_actual_groups.
  ...
  def group_body(local_group_id, num_groups_to_zero):
    global_group_id = local_group_id + group_offset
    should_copy_int = (lhs_group_sizes_ref[global_group_id] == 0).astype(jnp.int32)
    fill_zero(local_group_id, should_copy_int)
    return num_groups_to_zero + should_copy_int

  num_groups_to_zero = jax.lax.fori_loop(
      0, num_actual_groups, group_body, jnp.int32(0)
  )
