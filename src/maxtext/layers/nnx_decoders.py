...
class NNXDecoder(nnx.Module):
  ...
  def _apply_layers_sequentially(
    ...
    # upstream skip_block_remat: the scanned module applies its own remat internally; wrapping the
    # whole body again would double-remat and recompute the entire block.
    ...
    if skip_block_remat or _handwritten_owns_remat:
    ...
    if use_kv:
      ...
      if _xl:
        ...
        (final_carry, _dummy_final), scanned_state = jax.lax.scan(
            layer_fn_wrapped, (x_in, _dummy0), (params, state, (_w0_prev, _w1_prev), _is_top), unroll=unroll
        )
      else:
